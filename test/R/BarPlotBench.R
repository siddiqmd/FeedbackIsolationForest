...
pdf(file = 'summary/AllBars_Bench_lossTypes_lrates_mrdsc.pdf', width = 16, height = 9)
...
for(dataset in datasets){
  ...
  for(bid in 901:1200){
    ...
    resFinal[cIdx] <- resFinal[cIdx] + mean(X[ , numFeed])
            ...
            for(lrate in lrates){
              for(pwt in pwts){
                for(inwgt in inwgts){
                  ...
                  for(rtype in rtypes){
                    X <- read.csv(paste0('../benchmark/', dataset, '/out/', dataset, '_', bid, '_summary_feed_', numFeed, '_losstype_', lossType, '_updatetype_', 
