...
lnames <- #as.vector(t(outer(
          #  as.vector(t(outer(
              as.vector(t(outer(
                lossTypes, 1-lrates, paste, sep='-')))#,
                  # lrates, paste, sep='-'))),
                  #   pwts, paste, sep='-')))
cols <- rep(c('red','red','green','green','blue','blue'), 1)
ltypes <- rep(c(1,6),3)

pdf(file = paste0('pdf/Result_var_learning_rate.pdf'), width = 8, height = 8)
for(dataset in c("ann_thyroid_1v3", "cardiotocography_1", "mammography", "yeast", "abalone")){
  for(updType in updTypes){
    X <- read.csv(paste0('../out/', dataset, '_1_summary_feed_0_losstype_linear_updatetype_', updType, '_ngrad_1_reg_0_lrate_0_pwgt_0_inwgt_0.csv'))
