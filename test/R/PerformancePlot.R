
updTypes <- c('online', 'stochastic', 'batch')[1]
lossTypes <- c('linear', 'loglikelihood', 'logistic')
regs <- c('0', '0.0001', '0.001', '0.01', '0.1', '0.5')[c(1,4,6)]
lnames <- as.vector(t(outer(lossTypes, regs, paste, sep='-')))
cols <- c(rep(c('red'),length(regs)), rep(c('green'),length(regs)), rep(c('blue'),length(regs)))
ltypes <- rep(c(1,2,6),3)

pdf(file = paste0('pdf/Result.pdf'), width = 8, height = 8)
for(dataset in c("ann_thyroid_1v3", "cardiotocography_1", "mammography", "yeast", "abalone")){
  for(updType in updTypes){
    X <- read.csv(paste0('../out/', dataset, '_1_summary_feed_0_losstype_linear_updatetype_', updType, '_ngrad_1_reg_0.csv'))
    avg <- apply(X[ ,-1], 2, mean)
    X <- read.csv(paste0('../datasets/anomaly/', dataset, '/fullsamples/', dataset, '_1.csv'))
    plot(x = 1:100, y = avg, ylim = c(0,100), type = 'l', lty=1, col='black',
         xlab = '# Feedback', ylab = '# Anomaly', 
         main = paste0(dataset, ', ', updType, ', # Anomaly = ', length(which(X[ ,1]=='anomaly')), ', # Inst = ', nrow(X)))
    abline(a = 0, b = 1, lty = 3)
    cnt <- 0
    for(lossType in lossTypes){
      for(reg in regs){
        for(ng in 1){
          X <- read.csv(paste0('../out/', dataset, '_1_summary_feed_100_losstype_', lossType, '_updatetype_', updType, '_ngrad_', ng, '_reg_', reg, '.csv'))
          avg <- apply(X[ ,-1], 2, mean)
          cnt <- cnt + 1
          lines(x = 1:100, y = avg, col=cols[cnt], lty=ltypes[cnt])
        }
      }
    }
    legend(0, 100, legend=lnames, col=cols, lty = ltypes, bty = "n")
  }
}
dev.off()
