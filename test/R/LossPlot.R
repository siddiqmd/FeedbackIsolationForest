...
for(dataset in c( "abalone", "yeast", "ann_thyroid_1v3", "cardiotocography_1", "mammography")[1:2]){
  for(ng in c(1,10)){
    pdf(file = paste0('All_Loss_', dataset, '_ngrad_', ng, '.pdf'), width = 16, height = 8)
    par(mfrow=c(1,2))
    for(lossType in c('linear', 'loglikelihood')){
      regs = c('0', '0.0001', '0.001')
      if(lossType == 'linear'){
        regs = c('0', '0.1', '0.5')
      }
      for(reg in regs){
