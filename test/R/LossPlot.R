...
pdf(file = paste0('pdf/LossPlots_lossTypes_updTypes_numUpdate.pdf'), width = 16, height = 8)
par(mfrow=c(1,2))
for(dataset in c("ann_thyroid_1v3", "cardiotocography_1", "mammography", "abalone", "yeast")){
  for(lossType in c('linear', 'loglikelihood', 'logistic')){
    for(reg in c('0', '0.0001', '0.001', '0.01', '0.1', '0.5')[1]){
          ...
          for(pwt in c(0,1)[1]){
            for(ng in c(1,10)){
              X <- read.csv(paste0('../out/', dataset, '_1_cost_feed_100_losstype_', lossType, '_updatetype_', updType,
                                   '_ngrad_', ng, '_reg_', reg, '_lrate_', lrate, '_pwgt_', pwt, '_inwgt_', pwt, '.csv'))
              for(iter in 0){
                ...
                plot(x = 1:100, y = X[iter*4+1, -1], ylim = c(MIN,MAX), type = 'l', lty=1, col='black', xlab = '# Feedback', ylab = 'Loss', 
                     main = paste0(dataset, '-', lossType, '-', updType, '-numUpdate=', ng))#, '-', ng))
              ...
              X <- read.csv(paste0('../out/', dataset, '_1_summary_feed_0_losstype_', lossType, '_updatetype_', updType,
                                   '_ngrad_', ng, '_reg_', reg, '_lrate_', lrate, '_pwgt_', pwt, '_inwgt_', pwt, '.csv'))
